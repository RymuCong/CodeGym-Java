...
<body>
			...
			<table class="table table-hover">
				...
				<tbody>
					...
					for (Cart c : listOfCart) {
						Product prod = productService.getProductById(c.getProductId());
					%>
					<tr class="text-center">
						<td><img src="<%=prod.getProductImages()%>"
							style="width: 50px; height: 50px; width: auto;"></td>
						<td class="text-start"><%=prod.getProductName()%></td>
						<td><%=PriceFormatter.formatPrice(prod.getProductPriceAfterDiscount())%></td>
						<td><a
							href="CartServlet?cid=<%=c.getCartId()%>&opt=1"
						...
						<div class="qty"><%=c.getQuantity()%></div>
							<%if(c.getQuantity() > 1){ %>
							<a href="CartServlet?cid=<%=c.getCartId()%>&opt=2"
						...
						<td><%=PriceFormatter.formatPrice(c.getQuantity() * prod.getProductPriceAfterDiscount())%></td>
						<td><a
							href="CartServlet?cid=<%=c.getCartId()%>&opt=3"
					...
					<tr>
						<td class="text-end" colspan="8"><h4 class='pe-5'>
								Total Amount :
								<%=PriceFormatter.formatPrice(totalPrice)%></h4></td>
					</tr>
				</tbody>
			</table>
			<div class="text-end">
	...
	<script>
		$(document).ready(function(){
			...
			session.setAttribute("form", "cart");
			%>	
			});
		});
	</script>
